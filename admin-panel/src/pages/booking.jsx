...
const Booking = () => {
  ...
  const fetchDrivers = async () => {
    ...
    setDrivers(res.data.filter((d) => d.status === "online" && d.payactive === "Active"));
  };
  ...
  const openEdit = (b) => {
    ...
    setPickup(b.pickup); setDrop(b.drop); setStatus(b.status); setDriver(b.driver || "");
    setShowForm(true);
  };

  const filtered = bookings.filter(
    (b) => b.name.toLowerCase().includes(search.toLowerCase()) || b.mobile.toString().includes(search),
  );

  const submitForm = async () => {
    await axios.put(`${BASE_URL}/api/bookings/${editId}`, { driver, status });
    setShowForm(false); fetchBookings();
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.statsGrid}>
        {[
          ...
          <div key={s.label} style={styles.statCard}>
            <div style={{ ...styles.statIconWrapper, background: s.grad }}><span style={styles.statIcon}>{s.icon}</span></div>
            <div style={styles.statContent}><p style={styles.statLabel}>{s.label}</p><h3 style={styles.statValue}>{s.value}</h3></div>
          </div>
        ))}
      </div>
      ...
      <div style={styles.searchSection}>
        <div style={styles.searchWrapper}>
          ...
          <input placeholder="Search by name or mobile number..." value={search}
            onChange={(e) => setSearch(e.target.value)} style={styles.searchInput} />
        </div>
        <div style={styles.resultCount}>Showing <strong>{filtered.length}</strong> of <strong>{totalBookings}</strong> bookings</div>
      </div>
      ...
      <div style={styles.tableCard}>
        ...
        <div style={styles.tableWrapper}>
          <table style={styles.table}>
            <thead>
              <tr>{["ID","Customer Name","Mobile","Pickup Location","Drop Location","Assigned Driver","Status","Actions"].map((h) => <th key={h} style={styles.th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan="8" style={styles.noData}><div style={styles.noDataContent}><span style={styles.noDataIcon}>📭</span><p>No bookings found</p></div></td></tr>
              ) : filtered.map((b, index) => (
                <tr key={b.id} style={index % 2 === 0 ? styles.evenRow : styles.oddRow}>
                  <td style={styles.td}><span style={styles.idBadge}>{b.id}</span></td>
                  <td style={styles.td}><div style={styles.nameCell}><div style={styles.avatar}>{b.name?.charAt(0).toUpperCase()}</div><span style={styles.nameText}>{b.name}</span></div></td>
                  <td style={styles.td}>{b.mobile}</td>
                  <td style={styles.td}><div style={styles.locationCell}><span style={styles.locationIcon}>📍</span>{b.pickup}</div></td>
                  <td style={styles.td}><div style={styles.locationCell}><span style={styles.locationIcon}>🎯</span>{b.drop}</div></td>
                  <td style={styles.td}><span style={b.driver ? styles.driverAssigned : styles.driverNotAssigned}>{b.driver || "Not Assigned"}</span></td>
                  <td style={styles.td}><span style={getStatusStyle(b.status)}>{b.status || "Pending"}</span></td>
                  <td style={styles.td}><button style={styles.editBtn} onClick={() => openEdit(b)}>✏️ Assign</button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div style={styles.overlay}>
          <div style={styles.modalForm}>
            <div style={styles.modalHeaderForm}>
              <div style={styles.headerContent}><span style={styles.headerIcon}>🚗</span><span style={styles.headerText}>Assign Driver to Booking</span></div>
              <span style={styles.closeIconForm} onClick={() => setShowForm(false)}>✖</span>
            </div>
            <div style={styles.formGrid}>
              <div style={styles.sectionHeader}>Booking Details</div>
              <div style={styles.sectionDivider} />
              {[["Customer Name", name], ["Mobile Number", mobile], ["Pickup Location", pickup], ["Drop Location", drop]].map(([label, value]) => (
                <div key={label} style={styles.inputWrapper}>
                  <label style={styles.label}>{label}</label>
                  <input value={value} readOnly style={styles.formInput} />
                </div>
              ))}
              ...
              <div style={styles.inputWrapper}>
                ...
                <select value={driver} onChange={(e) => setDriver(e.target.value)} style={styles.formSelect}>
              ...
              <div style={styles.inputWrapper}>
                ...
                <input value={status} readOnly style={styles.formInput} />
              </div>
            </div>
            <div style={styles.btnRowForm}>
              <button style={styles.cancelBtnForm} onClick={() => setShowForm(false)}>Cancel</button>
              <button style={styles.saveBtnForm} onClick={submitForm}>Assign Driver</button>
            </div>
          </div>
        </div>
      )}
...
const styles = {
  ...
  statIcon:  { fontSize: "28px" }, statContent: { flex: 1 },
  ...
  evenRow: { background: "#fff" }, oddRow: { background: "#f9fafb" },
  ...
  locationCell: { display: "flex", alignItems: "center", gap: "6px" }, locationIcon: { fontSize: "14px" },
  ...
  noData: { padding: "60px 20px", textAlign: "center" }, noDataContent: { display: "flex", flexDirection: "column", alignItems: "center", gap: "12px", color: "#94a3b8" }, noDataIcon: { fontSize: "48px", opacity: 0.5 },
  ...
  modalForm: { width: "700px", maxHeight: "90vh", borderRadius: "20px", overflow: "hidden", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.4)", background: "#fff", display: "flex", flexDirection: "column" },
  ...
  headerContent: { display: "flex", alignItems: "center", gap: "12px" }, headerIcon: { fontSize: "28px" }, headerText: { fontSize: "24px", fontWeight: 700 },
  ...
  sectionHeader: { gridColumn: "1/-1", fontSize: "16px", fontWeight: 700, color: "#1e40af", marginTop: "15px", marginBottom: "-10px" },
  sectionDivider: { gridColumn: "1/-1", height: "2px", background: "linear-gradient(90deg,#2563eb,transparent)", marginBottom: "5px" },
  inputWrapper: { display: "flex", flexDirection: "column", gap: "8px" }, label: { fontSize: "13px", fontWeight: 600, color: "#374151" },
