  ...
  return (
    <div style={s.requestCard}>
      ...
      <div style={s.timerBarBg}>
        <div style={{ ...s.timerBarFill, width: `${pct}%`, backgroundColor: timer > 10 ? "#2563EB" : "#EF4444" }} />
      </div>

      <div style={s.requestHeader}>
        <div style={s.requestBadge}>
          <span style={s.requestBadgeText}>🔔 New Ride Request</span>
        </div>
        <div style={s.timerCircle}>
          <span style={{ fontSize: 18, fontWeight: 800, color: timer > 10 ? "#2563EB" : "#EF4444" }}>{timer}s</span>
        </div>
      </div>
      ...
      <div style={s.passengerRow}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={s.metaRow}>
            ...
            <span style={s.metaTag}>📞 {ride.customer_mobile || ride.phone || "—"}</span>
          </div>
        </div>
      </div>
      ...
      <div style={s.actionRow}>
        ...
        <button style={{ ...s.acceptBtn, opacity: accepting ? 0.7 : 1 }} onClick={handleAccept} disabled={accepting}>
...
export const ActiveTripCard = ({ ride, onComplete, role = "driver" }) => {
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      // Default center - pickup location or Chennai
      ...
      if (defLat && defLng) L.marker([defLat, defLng], { icon: pickupIcon }).addTo(map).bindPopup("📍 Pickup");
  ...
  /* ── Driver: send real location to server + update map ── */
  useEffect(() => {
  ...
  /* ── Customer: poll driver location every 4s ── */
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res  = await fetch(`${BASE_URL}/api/drivers?id=${driverId}`);
        const data = await res.json();
        const dr   = Array.isArray(data) ? data.find((d) => d.id == driverId) : null;
        if (dr?.lat && dr?.lng && driverMark.current && leafletMap.current) {
          driverMark.current.setLatLng([dr.lat, dr.lng]);
          leafletMap.current.setView([dr.lat, dr.lng], 15);
          setDriverPos({ lat: dr.lat, lng: dr.lng });
        }
      } catch {}
    };
  ...
  const statusLabel = ride.status === "accepted" ? "🟡 Driver Coming" :
                      ride.status === "inride"   ? "🟢 In Ride"       : "⏳ Assigned";
  const statusColor = ride.status === "inride"   ? "#10B981" : "#F59E0B";

  return (
    <div style={s.activeTripCard}>
      {/* Status badge */}
      <div style={{ ...s.statusBadge, backgroundColor: ride.status === "inride" ? "#D1FAE5" : "#FEF3C7" }}>
        <div style={{ ...s.statusDot, backgroundColor: statusColor }} />
        <span style={{ ...s.statusBadgeText, color: statusColor }}>{statusLabel}</span>
        {ride.status === "accepted" && (
          <span style={s.etaText}>ETA ~2 min</span>
        )}
      </div>
      ...
      <div style={s.personRow}>
        <div style={s.avatar}><span style={{ fontSize: 24 }}>{role === "driver" ? "👤" : "👨‍✈️"}</span></div>
        <div style={{ flex: 1 }}>
...
const s = {
  ...
  actionRow:  { display: "flex", gap: 10, padding: "0 16px 16px" },
  ...
  statusBadge:     { display: "flex", alignItems: "center", gap: 8, padding: "12px 16px", backgroundColor: "#F0FDF4" },
