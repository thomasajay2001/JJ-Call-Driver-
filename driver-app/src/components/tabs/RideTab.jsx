...
function calcFare(hrs, baseHrs, baseFare, extraHr) {
  if (!hrs || hrs <= 0) return 0;
  if (hrs <= baseHrs) return baseFare;
  return baseFare + Math.ceil(hrs - baseHrs) * extraHr;
}
...
const PaymentReceipt = ({ booking, onContinue }) => {
  ...
  return (
    <div style={PR.page}>
      ...
      {/* ── CTA ── */}
      <button style={PR.btn} onClick={onContinue}>
        ⭐ Rate My Ride
      </button>
    </div>
  );
...
const RideTab = () => {
  ...
  /* ── fare rates from master_settings (driver popup) ── */
  ...
  /* ── customer screens: receipt → rating ── */
  const [showPayReceipt,   setShowPayReceipt]   = useState(false);
  const [payReceiptData,   setPayReceiptData]   = useState(null);   // full booking with payment fields
  ...
  const receiptShown = useRef(false);  // guard — show receipt only once per completion
  ...
  /* ── fetch master_settings rates (driver) ── */
  useEffect(() => {
    ...
    try {
      ...
      if (currentRole === "customer") {
        ...
        if (unrated && wasActive && !receiptShown.current) {
          ...
          /* fetch full booking detail (includes payment fields) */
          try {
            ...
            setRatingBooking(full);  // queue for after receipt
          } catch {
        ...
        /* stay on receipt / rating while open */
  ...
  /* reset receiptShown when customer becomes active again */
  useEffect(() => {
    if (prevStatus.current && ["pending","assigned","accepted","inride"].includes(prevStatus.current)) {
  ...
  const openCompletePopup = () => {
    setRideToComplete(booking); setHours(""); setPopupErr(""); setShowPopup(true);
  };
  ...
  const handleComplete = async () => {
    ...
    setCompleting(true);
    try {
      const fare = calcFare(h, rateBaseHrs, rateBaseFare, rateExtraHr);
      await axios.post(`${BASE_URL}/api/complete-ride`,{
        bookingId:    rideToComplete.id,
        driverId:     DRIVER_ID,
        amount:       fare,
        ride_hours:   Math.floor(h),
        ride_minutes: Math.round((h - Math.floor(h)) * 60),
      });
      if (watchId.current) navigator.geolocation.clearWatch(watchId.current);
  ...
  const liveFare  = calcFare(parsedHrs, rateBaseHrs, rateBaseFare, rateExtraHr);
  const extraHrs  = parsedHrs > rateBaseHrs ? Math.ceil(parsedHrs - rateBaseHrs) : 0;
  const isExtra   = parsedHrs > rateBaseHrs;
  ...
  if (role === "driver") {
    ...
    return (
      <div style={S.page}>
        ...
        {activeTab === "history" && <HistoryPanel bookings={filteredBookings} summary={summary} loading={histLoading} dateMode={dateMode} setDateMode={setDateMode} customDate={customDate} setCustomDate={setCustomDate} customMonth={customMonth} setCustomMonth={setCustomMonth} statusFilter={statusFilter} setStatusFilter={setStatusFilter} searchQuery={searchQuery} setSearchQuery={setSearchQuery} expandedId={expandedId} setExpandedId={setExpandedId} onRefresh={fetchHistory} role="driver" onGoActive={()=>setActiveTab("active")}/>}
        ...
        {showPopup && (
          <div style={P.overlay}>
            <div style={P.sheet}>
              ...
              {/* live rates badge */}
              <div style={{display:"flex",alignItems:"center",gap:8,backgroundColor:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:10,padding:"8px 12px",marginBottom:14}}>
                <span style={{fontSize:14}}>💰</span>
                <span style={{fontSize:12,fontWeight:600,color:"#166534"}}>₹{rateBaseFare} for {rateBaseHrs} hr{rateBaseHrs!==1?"s":""} · +₹{rateExtraHr}/hr extra</span>
                <span style={{marginLeft:"auto",fontSize:10,fontWeight:700,color:"#22C55E",background:"#DCFCE7",padding:"2px 7px",borderRadius:20}}>LIVE</span>
              </div>
              <div style={P.structCard}>
                <div style={P.structRow}>
                  <div style={P.structLeft}><span style={P.structIcon}>📦</span><div><p style={P.structTitle}>Base Package</p><p style={P.structDesc}>Up to {rateBaseHrs} hours</p></div></div>
                  <span style={P.structPrice}>₹{rateBaseFare}</span>
                </div>
                <div style={P.structDivider}/>
                <div style={P.structRow}>
                  <div style={P.structLeft}><span style={P.structIcon}>⏰</span><div><p style={P.structTitle}>Additional Hours</p><p style={P.structDesc}>After {rateBaseHrs} hrs</p></div></div>
                  <span style={P.structPrice}>₹{rateExtraHr}/hr</span>
                </div>
              </div>
              ...
              {parsedHrs > 0 && (
                <div style={P.fareCard}>
                  ...
                  <div style={P.fareRow}><span style={P.fareKey}>Base ({rateBaseHrs} hrs package)</span><span style={P.fareAmt}>₹{rateBaseFare}</span></div>
                  {isExtra && <div style={P.fareRow}><span style={P.fareKey}>Extra {extraHrs} hr{extraHrs>1?"s":""} × ₹{rateExtraHr}</span><span style={P.fareAmt}>₹{extraHrs*rateExtraHr}</span></div>}
                  ...
                  <p style={P.fareExample}>{isExtra?`₹${rateBaseFare} + ${extraHrs} × ₹${rateExtraHr} = ₹${liveFare}`:`Flat package — up to ${rateBaseHrs} hrs = ₹${rateBaseFare}`}</p>
                </div>
              )}
              <div style={P.actions}>
  ...
  /* ── STEP 1: Payment Receipt (shows first after completion) ── */
    ...
    return (
      <PaymentReceipt
        ...
        onContinue={() => {
          ...
          /* ratingBooking is already set — rating screen renders next */
        }}
      />
    );
  }

  /* ── STEP 2: Rating screen (shows after receipt is dismissed) ── */
  ...
  return (
    <div style={S.page}>
      ...
      {activeTab==="history" && <HistoryPanel bookings={filteredBookings} summary={summary} loading={histLoading} dateMode={dateMode} setDateMode={setDateMode} customDate={customDate} setCustomDate={setCustomDate} customMonth={customMonth} setCustomMonth={setCustomMonth} statusFilter={statusFilter} setStatusFilter={setStatusFilter} searchQuery={searchQuery} setSearchQuery={setSearchQuery} expandedId={expandedId} setExpandedId={setExpandedId} onRefresh={fetchHistory} role="customer" onGoActive={()=>setActiveTab("active")}/>}
...
const P = {
  ...
  structCard:    {backgroundColor:"#F8FAFC",border:"1.5px solid #E2E8F0",borderRadius:14,padding:"12px 14px",marginBottom:16},
